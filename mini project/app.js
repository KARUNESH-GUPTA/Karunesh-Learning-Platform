...
function handleLogin() {
    ...
    showSuccessMessage('✅ Login Successful!', `Welcome back, ${email.split('@')[0]}!`);
...
function handleSignup() {
    ...
    // Show success message
    showSuccessMessage('🎉 Account Created Successfully!', `Welcome to CyberMastery Hub, ${name}!`);
...
function showSuccessMessage(title, message) {
    ...
    successBox.innerHTML = `
        ...
        <h3 style="color: #21808d; margin-bottom: 0.5rem; font-size: 1.5rem;">${title}</h3>
        <p style="color: #626c71; margin: 0;">${message}</p>
    `;
...
document.addEventListener('click', function(event) {
    ...
    if (authModal && event.target === authModal) {
        // Only close if user has logged in (for demo, we'll allow closing)
        // hideAuthModal();
    }
});

// Prevent form submission on Enter key (optional enhancement)
document.addEventListener('keypress', function(event) {
    if (event.key === 'Enter' && document.getElementById('authModal').classList.contains('active')) {
        const loginFormActive = document.getElementById('loginForm').classList.contains('active');
        if (loginFormActive) {
            handleLogin();
        } else {
            handleSignup();
